...
#
# TODO: Can we at some point advertise our abilities via
#       OpenMath directly 
...
# Returns the first global variable that this is assigned
# to
# TODO: This used to have a comment about fixing it
InstallGlobalFunction(GVarNameForObject,
function( obj )
    local n, res;
...
InstallGlobalFunction(FiltersForOperation,
function( oper )
    local res, res2, filts, opid, fset, flags;
...
InstallGlobalFunction(GAPAndFilterUnpack,
function(t)
    local res;
...
InstallGlobalFunction(GAPFilterToFilterType
function(fid)
    if INFO_FILTERS[fid] in FNUM_CATS then
        return "GAP_Category";
    elif INFO_FILTERS[fid] in FNUM_REPS then
...
InstallGlobalFunction(GAPFilterInfo,
function(res, name, f)
    local lres, ff, i;
...
InstallGlobalFunction(GAPTypesInfoByGlobalVariables,
function()
    local name, names, n, obj, res, lres, f;
...
# Make GAP Type graph as a record
InstallGlobalFunction(GAPTypesInfo,
function()
    local res, lres, i, j, f, ff, a, meths, mpos, objs, m, mres, n, t, notcovered, v;
...
# Write the graph of type info to JSon file
InstallGlobalFunction(GAPTypesToJson,
function(file)
    local fd, n, typeinfo;
