...
> lovely!  All done!
> """);;
gap> MitM_OMRecToGAPFunc(MitM_ReadSCSCP(stream).result);
...
> <?scscp end ?>
> """);;
gap> p := MitM_OMRecToGAPFunc(MitM_ReadSCSCP(stream).result);;
...
> <?scscp end ?>
> """);;
gap> MitM_OMRecToGAPFunc(MitM_ReadSCSCP(stream).result).result;
42
